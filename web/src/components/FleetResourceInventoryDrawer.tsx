...
const FleetResourceInventoryDrawer = ({
  ...
  const { t } = useLang();
  const [selectedInstanceIdsOverride, setSelectedInstanceIds] = useState<string[] | null>(null);
  ...
  const availableInstanceIds = instances.map((instance) => instance.name);
  const selectedInstanceIds =
    selectedInstanceIdsOverride === null
      ? availableInstanceIds
      : selectedInstanceIdsOverride.filter((instanceId) =>
          availableInstanceIds.includes(instanceId),
        );
  ...
  const loadInventory = async () => {
    ...
    try {
      ...
      if (requestId === requestIdRef.current) {
        message.error(t('fleetInventory.loadFailed'));
      }
    } finally {
              ...
              <Space wrap>
                ...
                <Select
                  ...
                  options={[
                    ...
                    { value: 'APACHE', label: t('instance.openSourceEdition') },
                    { value: 'ALIYUN', label: t('instance.aliyunEdition') },
                    { value: 'TENCENT', label: t('instance.tencentEdition') },
                  ]}
                />
                <Checkbox
                  checked={sharedOnly}
