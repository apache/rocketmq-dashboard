...
interface Props {
  ...
  onApplyNamedQuery?: (query: SavedMessageQuery) => void;
...
const MessageQueryHistoryDrawer = ({
  ...
  const { t } = useLang();
  const [tab, setTab] = useState<'messages' | 'traces' | 'named'>('messages');
  ...
  return (
    ...
    >
      {tab !== 'named' && (
        <>
          <Flex gap={32} style={{ marginBottom: 16 }}>
            ...
            />
            <Statistic
              title={t('messageHistory.traceQueries')}
      ...
      <Tabs
        ...
        onChange={(key) => {
          setPage(1);
          setTab(key as 'messages' | 'traces' | 'named');
        }}
        items={[
          ...(onApplyNamedQuery
            ? [
