...
export async function listAlertRuleAssets(): Promise<AlertRuleAssetInfo[]> {
  const res = await client.get<{ data: AlertRuleAssetInfo[] }>('/alert-rules/assets');
  return res.data.data;
}

export async function getAlertRuleAsset(name: string): Promise<string> {
  const res = await client.get<{ data: string }>(`/alert-rules/assets/${encodeURIComponent(name)}`);
  return res.data.data;
}

export async function exportAlertRuleAsset(name: string): Promise<Blob> {
  const res = await client.get<Blob>(`/alert-rules/assets/${encodeURIComponent(name)}/export`, {
    responseType: 'blob',
  });
  return res.data;
}
