...
describe('message API', () => {
  ...
  it('unwraps trace records with numeric timestamps', async () => {
    ...
    mock
      .onGet('/messages/msg-1/trace', { params: { instanceId: '1', topic: 'orders' } })
      .reply(200, { code: 200, data: trace });

    await expect(getMessageTrace('msg-1', 1, 'orders')).resolves.toEqual(trace);
  });
  ...
  it('encodes message IDs before requesting trace records', async () => {
    ...
    mock
      .onGet('/messages/AC1E0A64%2F0000%202A9F%3A1/trace', {
        params: { instanceId: '1', topic: 'orders' },
      })
      .reply(200, { code: 200, data: trace });

    await expect(getMessageTrace('AC1E0A64/0000 2A9F:1', 1, 'orders')).resolves.toEqual(trace);
  });
});
