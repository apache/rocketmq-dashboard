...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'instance.accessTypeRequired': { zh: '请选择接入方式', en: 'Select the access type' },
  'instance.credentialRefExample': { zh: '例：production-admin', en: 'e.g. production-admin' },
  'instance.credentialRefHelp': { zh: '可选。仅保存服务端配置中的凭据引用，不会保存或传输 AK/SK。', en: 'Optional. Only the credential reference in the server config is stored; AK/SK is never saved or transmitted.' },
  'instance.credentialRefLabel': { zh: '管理凭据引用', en: 'Admin Credential Reference' },
  'instance.endpointExampleDirect': { zh: '例：namesrv.mq.svc.cluster.local:9876', en: 'e.g. namesrv.mq.svc.cluster.local:9876' },
  'instance.endpointExampleProxy': { zh: '例：proxy.mq.svc.cluster.local:8080', en: 'e.g. proxy.mq.svc.cluster.local:8080' },
  'instance.endpointExtraDirect': { zh: 'Direct 模式请填写 NameServer SLB 地址（K8s 场景下一般为 NameServer Service 地址，如 namesrv.mq.svc:9876）', en: 'For Direct mode, enter the NameServer SLB address (typically the NameServer Service address in K8s, e.g. namesrv.mq.svc:9876)' },
  'instance.endpointExtraProxyCluster': { zh: 'Proxy Cluster 模式请填写独立 Proxy 集群的 SLB 内网地址（如 proxy.mq.svc:8080）', en: 'For Proxy Cluster mode, enter the SLB intranet address of a standalone Proxy cluster (e.g. proxy.mq.svc:8080)' },
  'instance.endpointExtraProxyLocal': { zh: 'Proxy Local 模式请填写与 Broker 同进程部署的 Proxy 接入地址（如 broker-proxy.mq.svc:8080）', en: 'For Proxy Local mode, enter the access address of the Proxy co-deployed with the broker (e.g. broker-proxy.mq.svc:8080)' },
  'instance.endpointExtraSelectFirst': { zh: '请先选择接入方式', en: 'Select the access type first' },
  'instance.endpointRequired': { zh: '请输入接入地址', en: 'Enter the access address' },
  'instance.endpointTooltip': { zh: '接入地址为客户端访问入口，会展示在 Topic 等页面供客户端配置使用。若客户端环境无法解析该地址（如 K8s 内部 Service 域名），可自行配置 DNS 解析或在客户端 hosts 中映射。', en: 'The access address is the client entry point, shown on pages such as Topic for client configuration. If the client cannot resolve it (e.g. an in-cluster K8s Service name), configure DNS or map it in the client hosts file.' },
  'instance.instanceIdRequired': { zh: '请输入实例 ID', en: 'Enter the instance ID' },
  'instance.instanceIdTooLong': { zh: '实例 ID 不能超过 64 个字符', en: 'Instance ID cannot exceed 64 characters' },
  'instance.remarkPlaceholder': { zh: '可选，描述实例用途', en: 'Optional: describe the instance purpose' },
