...
const translations: Record<string, Record<Lang, string>> = {
  ...
  'topic.addrCount': { zh: '{count} 个地址', en: '{count} address(es)' },
  'topic.brokerCountStat': { zh: 'Broker 数', en: 'Brokers' },
  'topic.diagItem': { zh: '诊断项', en: 'Diagnostics' },
  'topic.routeCritical': { zh: '异常', en: 'Critical' },
  'topic.routeWarning': { zh: '关注', en: 'Warning' },
  'topic.diagSummaryGood': { zh: '共 {brokers} 个 Broker，写队列 {write} 个，读队列 {read} 个。', en: '{brokers} broker(s) total, {write} write queue(s) and {read} read queue(s).' },
  'topic.diagSummaryIssues': { zh: '发现 {count} 个诊断项，优先处理异常标记的 Broker。', en: 'Found {count} diagnostic item(s); prioritize brokers marked as critical.' },
  'topic.diagTitle': { zh: '路由诊断：{status}', en: 'Route diagnostics: {status}' },
  'topic.queueBalanced': { zh: '队列均衡', en: 'Queues balanced' },
  'topic.queueSkew': { zh: '队列差距 写 {w} / 读 {r}', en: 'Queue skew — write {w} / read {r}' },
  'topic.readQueueCount': { zh: '{count} 个读队列', en: '{count} read queue(s)' },
  'topic.readableBrokers': { zh: '可读 Broker', en: 'Readable Brokers' },
  'topic.rebuildOnBroker': { zh: '在 Broker 上重建', en: 'Rebuild on Brokers' },
  'topic.replicaCountStat': { zh: 'Replica 数', en: 'Replicas' },
  'topic.routeInfo': { zh: '路由信息', en: 'Route Information' },
  'topic.suggestAction': { zh: '建议处理', en: 'Suggested Actions' },
  'topic.writeQueueCount': { zh: '{count} 个写队列', en: '{count} write queue(s)' },
  'topic.writableBrokers': { zh: '可写 Broker', en: 'Writable Brokers' },
  ...
  'topicRoute.status.healthy': { zh: '路由健康', en: 'Route healthy' },
  'topicRoute.status.warning': { zh: '需要关注', en: 'Needs attention' },
  'topicRoute.status.critical': { zh: '不可用', en: 'Unavailable' },
  'topicRoute.noIssues': { zh: '无', en: 'None' },
  'topicRoute.issue.NO_ROUTE': { zh: 'Broker 上没有 Topic 路由', en: 'Broker has no Topic route' },
  'topicRoute.issue.MISSING_BROKER_ADDRESS': { zh: 'Broker 地址缺失', en: 'Broker address missing' },
  'topicRoute.issue.MISSING_MASTER_ADDRESS': { zh: 'Master 地址缺失', en: 'Master address missing' },
  'topicRoute.issue.WRITE_QUEUE_UNAVAILABLE': { zh: '写队列不可用', en: 'Write queue unavailable' },
  'topicRoute.issue.READ_QUEUE_UNAVAILABLE': { zh: '读队列不可用', en: 'Read queue unavailable' },
  'topicRoute.issue.PERMISSION_NOT_WRITABLE': { zh: '权限不允许写入', en: 'Write permission missing' },
  'topicRoute.issue.PERMISSION_NOT_READABLE': { zh: '权限不允许读取', en: 'Read permission missing' },
  'topicRoute.issue.READ_WRITE_QUEUE_MISMATCH': { zh: '读写队列不一致', en: 'Read/write queue mismatch' },
  'topicRoute.issue.WRITE_QUEUE_SKEW': { zh: '写队列分布不均', en: 'Write queue distribution skewed' },
  'topicRoute.issue.READ_QUEUE_SKEW': { zh: '读队列分布不均', en: 'Read queue distribution skewed' },
  'topicRoute.issue.DUPLICATE_BROKER_ADDRESS': { zh: 'Broker 地址重复', en: 'Duplicate broker address' },
  'topicRoute.issue.NO_WRITABLE_ROUTE': { zh: '没有可写路由', en: 'No writable route' },
  'topicRoute.issue.NO_READABLE_ROUTE': { zh: '没有可读路由', en: 'No readable route' },
  'topicRoute.issue.SINGLE_BROKER_ROUTE': { zh: '单 Broker 路由', en: 'Single-broker route' },
  'topicRoute.issueDesc.NO_ROUTE': { zh: '元数据中存在 Topic 记录，但当前实例没有返回任何 Broker 路由。', en: 'The Topic exists in metadata, but this instance returned no broker route.' },
  'topicRoute.issueDesc.MISSING_BROKER_ADDRESS': { zh: 'NameServer 返回了队列元数据，但没有返回可用于定位 Broker 的地址。', en: 'NameServer returned queue metadata but no address to locate the Broker.' },
  'topicRoute.issueDesc.MISSING_MASTER_ADDRESS': { zh: '该 Broker 只返回了非 master 地址，Topic 写入链路需要确认 master 是否在线。', en: 'Only non-master addresses were returned; confirm the master is online for the write path.' },
  'topicRoute.issueDesc.WRITE_QUEUE_UNAVAILABLE': { zh: '该 Broker 没有可写队列，生产者不会把消息写到这个 Broker。', en: 'No writable queue on this Broker; producers will not write to it.' },
  'topicRoute.issueDesc.READ_QUEUE_UNAVAILABLE': { zh: '该 Broker 没有可读队列，消费者不会从这个 Broker 拉取消息。', en: 'No readable queue on this Broker; consumers will not pull from it.' },
  'topicRoute.issueDesc.PERMISSION_NOT_WRITABLE': { zh: 'Topic 权限缺少写权限，生产者发送可能失败或被路由到其他 Broker。', en: 'Missing write permission; sends may fail or be routed elsewhere.' },
  'topicRoute.issueDesc.PERMISSION_NOT_READABLE': { zh: 'Topic 权限缺少读权限，消费者订阅后可能无法正常消费。', en: 'Missing read permission; subscribers may not consume normally.' },
  'topicRoute.issueDesc.READ_WRITE_QUEUE_MISMATCH': { zh: '该 Broker 的读队列数和写队列数不同，扩缩容或迁移后需要确认配置是否符合预期。', en: 'Different read and write queue counts; verify config after scaling or migration.' },
  'topicRoute.issueDesc.WRITE_QUEUE_SKEW': { zh: '不同 Broker 的写队列数差距较大，生产流量可能无法均匀分摊。', en: 'Large gaps between brokers; producer traffic may not spread evenly.' },
  'topicRoute.issueDesc.READ_QUEUE_SKEW': { zh: '不同 Broker 的读队列数差距较大，消费者负载可能无法均匀分摊。', en: 'Large gaps between brokers; consumer load may not spread evenly.' },
  'topicRoute.issueDesc.DUPLICATE_BROKER_ADDRESS': { zh: '多个 BrokerName 返回了相同地址，请确认 NameServer 注册信息是否过期。', en: 'Multiple BrokerNames share an address; check for stale NameServer registrations.' },
  'topicRoute.issueDesc.NO_WRITABLE_ROUTE': { zh: '所有 Broker 都缺少写权限或写队列，生产者无法向该 Topic 发送消息。', en: 'Every broker lacks write permission or queues; producers cannot send to this Topic.' },
  'topicRoute.issueDesc.NO_READABLE_ROUTE': { zh: '所有 Broker 都缺少读权限或读队列，消费者无法从该 Topic 拉取消息。', en: 'Every broker lacks read permission or queues; consumers cannot pull from this Topic.' },
  'topicRoute.issueDesc.SINGLE_BROKER_ROUTE': { zh: '该 Topic 只返回一个 Broker 路由，生产业务需要确认是否符合容灾预期。', en: 'Only one broker route; confirm this meets redundancy expectations for production.' },
  'topicRoute.rec.noRoute': { zh: '确认 Topic 已在目标 Broker 上创建；必要时使用“在 Broker 上重建”。', en: 'Create the Topic on the target Broker, or rebuild it there if needed.' },
  'topicRoute.rec.checkRegistration': { zh: '检查 Broker 是否仍向 NameServer 注册，并确认 master 节点可达。', en: 'Check whether Brokers still register with NameServer and the master node is reachable.' },
  'topicRoute.rec.ensureWritePerm': { zh: '确认 Topic 权限包含写权限，避免生产者发送失败。', en: 'Confirm the Topic has write permission to avoid producer send failures.' },
  'topicRoute.rec.ensureReadPerm': { zh: '确认 Topic 权限包含读权限，避免消费者订阅后无可读队列。', en: 'Confirm the Topic has read permission so subscribers have readable queues.' },
  'topicRoute.rec.alignQueueConfig': { zh: '对比各 Broker 上的 TopicConfig，统一读写队列数后再观察客户端路由。', en: 'Compare TopicConfig across Brokers and align read/write queue counts, then re-check routing.' },
  'topicRoute.rec.balanceQueues': { zh: '评估是否需要扩容、迁移或重新分配队列，降低单 Broker 负载集中风险。', en: 'Consider scaling, migrating or rebalancing queues to reduce load concentration on one Broker.' },
  'topicRoute.rec.confirmSingleBroker': { zh: '确认该 Topic 是否预期只部署在单 Broker；生产业务建议准备冗余路由。', en: 'Confirm whether single-broker deployment is intended; production traffic should have redundant routes.' },
  'topicRoute.rec.cleanupRegistration': { zh: '清理过期 Broker 注册信息，避免客户端拿到重复或错误地址。', en: 'Remove stale Broker registrations so clients do not receive duplicate or wrong addresses.' },
  // ─── BrokerCluster ───
};
