...
const K8sCertsPage = () => {
  ...
  useEffect(() => {
    ...
    listK8sCerts()
      ...
      .catch((error: unknown) => {
        if (active) message.error(getErrorMessage(error, t('cert.requestFailed')));
      })
      .finally(() => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      message.success(t('cert.certCreated', { name: created.k8sId }));
      setCreateModalOpen(false);
      createForm.resetFields();
    } catch (error: unknown) {
      message.error(getErrorMessage(error, t('cert.requestFailed')));
    } finally {
      setCreating(false);
    }
  };
    ...
    },
    {
      ...
      render: (_: unknown, cert: K8sCertInfo) => (
        <Popconfirm
          ...
          okText={t('cert.delete')}
          cancelText={t('cert.cancel')}
  ...
  return (
    <div style={{ padding: 24 }}>
      <PageHeader title={t('cert.pageTitle')} subtitle={t('cert.totalCount', { count: filteredCerts.length })} />
      <InfoBanner
      ...
      >
        <Form form={createForm} layout="vertical" preserve={false}>
          ...
          <Form.Item
            label="K8s 集群名称"
            name="cluster"
            rules={[{ required: true, message: t('cert.clusterNameRequired') }]}
          >
            <Input placeholder={t('cert.clusterNamePlaceholder')} />
          </Form.Item>
          <Form.Item label={t('cert.typeLabel')} name="type" initialValue="TLS">
            <Select
              virtual={false}
              options={[
                { value: 'TLS', label: 'TLS' },
          ...
          <Form.Item label={t('cert.keyPemLabel')} name="keyPem" extra={t('cert.keyPemHelp')}>
            <Input.TextArea
              rows={6}
