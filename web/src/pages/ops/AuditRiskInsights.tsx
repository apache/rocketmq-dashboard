...
// Not utils/format's formatPercent: that one uses toFixed and always emits a decimal, whereas
// shares read better without a trailing zero, matching the dashboard's traffic percentages.
  ...
  const renderIssue = (issue: AuditRiskIssue) => (
    ...
    >
      {t(issueTextKey(issue), {
        ...
        target: issue.target ?? '-',
        threshold: issue.threshold == null ? '-' : formatSharePercent(issue.threshold),
      })}
    </Tag>
  );
