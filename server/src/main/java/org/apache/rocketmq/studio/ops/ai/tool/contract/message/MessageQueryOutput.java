...
public record MessageQueryOutput(
    ...
    public static MessageQueryOutput fromPage(MessageQueryPageVO result, boolean includeBody) {
        return new MessageQueryOutput(result.getItems().stream()
                .map(message -> Item.from(message, includeBody)).toList(),
                result.getTotal(), result.getPage(), result.getSize(), result.isResultMayBeTruncated());
    }
    ...
    public static MessageQueryOutput fromUniqueKey(
        ...
        return new MessageQueryOutput(messages.subList(from, to).stream()
                .map(message -> Item.from(message, includeBody)).toList(),
                messages.size(), page, pageSize, false);
    }
    ...
    public record Item(
        ...
        static Item from(MessageRecordVO message, boolean includeBody) {
            return new Item(message.getMsgId(), message.getTopic(), message.getTag(), message.getKey(),
                    message.getStoreTime(), message.getStoreHost(), message.getBornHost(),
