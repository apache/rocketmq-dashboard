 ...
 * drawer so both apply the same {@value #MAX_PROPERTIES}-entry / {@value #MAX_PROPERTY_VALUE_CHARS}-code-point
...
public final class MessagePropertyDisplay {
    ...
    /**
     * True when any value exceeds {@link #MAX_PROPERTY_VALUE_CHARS} code points and would be
    ...
    public static boolean hasOversizedProperty(Map<String, String> properties) {
        return properties != null && properties.values().stream()
                .anyMatch(value -> value != null && codePointCount(value) > MAX_PROPERTY_VALUE_CHARS);
    }

    private static String abbreviate(String value) {
        if (value == null || codePointCount(value) <= MAX_PROPERTY_VALUE_CHARS) {
            return value;
        }
        ...
        int end = value.offsetByCodePoints(0, MAX_PROPERTY_VALUE_CHARS);
