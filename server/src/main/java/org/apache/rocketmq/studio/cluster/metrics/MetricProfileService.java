...
public class MetricProfileService {
    ...
    /**
     * Semantic metric that a rule's metric string refers to, matched against both the semantic key
     * and the active profile's exporter name. A rule may store either form, and guessing the
     * semantic metric for an exporter name that belongs to a different mapping yields label names
     * the referenced series do not carry. Returns empty when the active profile has no such metric.
     */
    public Optional<String> resolveSemanticMetric(String metric) {
        ...
        return listProfiles().get(0).getMetrics().stream()
                .filter(mapping -> normalized.equals(mapping.getSemanticMetric())
    ...
    private List<MetricProfileVO.MetricMappingVO> rocketmq5NativeMetrics() {
        return List.of(
                mapping(SemanticMetric.MESSAGE_IN_TPS, "rocketmq_messages_in_total",
                        ...
                        Map.of("cluster", "cluster", "broker", "node_id", "topic", "topic"),
