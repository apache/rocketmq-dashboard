...
public final class SqlLikeUtils {
     ...
     *
     * <p>{@code column} must be a compile-time literal: it is interpolated into the SQL text rather
     * than bound, so a caller must never pass a request parameter, a header or any other value it did
     * not choose itself. Every call site in this project passes a literal.
     *
     * @param column the column to match, as a compile-time literal, without quotes or a table alias
