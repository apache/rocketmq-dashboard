...
/**
 * Query a single message by its Message ID. Read-only and safe for AI/MCP/CLI callers;
 * delegates to the standard message service used by the web UI.
 */
@Component
@RequiredArgsConstructor
public class MessageQueryByIdToolHandler
        implements ToolHandler<MessageQueryByIdInput, ListOutput<MessageItem>> {

    private final MessageService messageService;

    @Override
    public String name() {
        return "rmq.message.query_by_id";
    }
    ...
    public ListOutput<MessageItem> execute(
            MessageQueryByIdInput input, ToolExecutionContext context) {
        return new ListOutput<>(messageService.queryMessages(
                        context.cluster(), input.topic(), input.msgId(), null, null, null, null)
                .stream()
