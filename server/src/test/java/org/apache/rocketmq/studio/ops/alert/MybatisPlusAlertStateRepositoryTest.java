...
class MybatisPlusAlertStateRepositoryTest {
    ...
    @Test
    void findsActiveStatesForRulesWithPaddedStoredInstanceIdTest() {
        // Firing rules whose stored instanceId has surrounding whitespace must still be
        // visible in the active-state view, matching NativeAlertRuleScopeMatcher.
        RmqAlertStateMapper mapper = mock(RmqAlertStateMapper.class);
        RmqAlertState state = new RmqAlertState();
        state.setRuleId(4L);
        state.setFingerprint("fingerprint");
