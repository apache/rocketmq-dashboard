...
class MessageQueryByKeyToolHandlerTest {

    @Mock
    private MessageService messageService;

    @InjectMocks
    private MessageQueryByKeyToolHandler handler;
    ...
    void executeShouldDelegateToMessageServiceAndProject() {
        ...
        when(messageService.queryMessages(
                ...
                eq("order-123"), isNull(), isNull()))
                .thenReturn(List.of(message));

        ListOutput<MessageItem> result = handler.execute(
                new MessageQueryByKeyInput(
                        "instance-a", "order-123", "TopicA", null, null),
                context("instance-a"));
