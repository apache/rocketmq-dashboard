...
/**
 * Unit tests for {@link AliyunCatalogController}: the cloud-catalog endpoints delegate to
 * the catalog service with the credential/region/search parameters intact.
 */
...
class AliyunCatalogControllerTest {
    ...
    @InjectMocks
    private AliyunCatalogController controller;

    @Test
    void listsRegionsForACredential() {
        List<CloudRegionVO> regions = List.of();
        when(catalogService.listRegions(9L)).thenReturn(regions);

        assertThat(controller.listRegions(9L).getData()).isSameAs(regions);
    ...
    void listsInstancesWithRegionAndSearch() {
        ...
        when(catalogService.listCloudInstances(9L, "cn-hangzhou", "prod")).thenReturn(instances);

        assertThat(controller.listInstances(9L, "cn-hangzhou", "prod").getData())
                .isSameAs(instances);
    ...
    void passesNullSearchThroughWhenAbsent() {
        when(catalogService.listCloudInstances(9L, "cn-hangzhou", null))
                .thenReturn(List.of());

        assertThat(controller.listInstances(9L, "cn-hangzhou", null).getData()).isEmpty();
