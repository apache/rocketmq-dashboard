...
未指定 `msgId` 或 `key` 的 Topic 扫描默认查询最近 1 小时，时间范围最大 7 天。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `title` | `string` | 节点类型，如 `produce` / `consume` / `endTransaction` / `recall` |
| `timestamp` | `number` | 事件时间（Unix 毫秒时间戳） |
| `status` | `string` | 状态，如 `finish` / `failed` |
