...
@Permission
public class ConsumerThreadPoolMonitorController {

    @Autowired
    private ConsumerThreadPoolMonitorService consumerThreadPoolMonitorService;

    @RequestMapping(value = "/threadPoolSaturation.query", method = RequestMethod.GET)
    @ResponseBody
    public ConsumerThreadPoolSaturationReport inspectThreadPool(
        @RequestParam("consumerGroup") String consumerGroup,
        @RequestParam(value = "clientId", required = false) String clientId) {
        return consumerThreadPoolMonitorService.inspectThreadPoolSaturation(consumerGroup, clientId);
    }
}
