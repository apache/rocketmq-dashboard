...
@Permission
public class BrokerDiskForecasterController {

    @Autowired
    private BrokerDiskForecasterService brokerDiskForecasterService;

    @RequestMapping(value = "/diskForecast.query", method = RequestMethod.GET)
    @ResponseBody
    public BrokerDiskWatermarkReport forecastDisk(
        @RequestParam(value = "clusterName", required = false) String clusterName) {
        return brokerDiskForecasterService.forecastDiskWatermarks(clusterName);
    }
}
