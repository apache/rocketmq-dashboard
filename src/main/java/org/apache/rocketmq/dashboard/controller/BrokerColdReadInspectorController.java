...
@Permission
public class BrokerColdReadInspectorController {

    @Autowired
    private BrokerColdReadInspectorService brokerColdReadInspectorService;

    @RequestMapping(value = "/coldRead.query", method = RequestMethod.GET)
    @ResponseBody
    public BrokerColdReadReport inspectColdRead(
        @RequestParam(value = "clusterName", required = false) String clusterName) {
        return brokerColdReadInspectorService.inspectColdDataRead(clusterName);
    }
}
