...
public class MQAdminExtImpl implements MQAdminExt {
    ...
    public TopicList fetchAllTopicList() throws RemotingException, MQClientException, InterruptedException {
        ...
        logger.debug("op=look={}", JsonUtil.objectToString(topicList.getTopicList()));
        return topicList;
    }
    ...
    public void deleteTopicInBroker(Set<String> addrs, String topic)
            throws RemotingException, MQBrokerException, InterruptedException, MQClientException {
        logger.info("addrs={} topic={}", JsonUtil.objectToString(addrs), topic);
    ...
    public MessageExt viewMessage(String topic,
        ...
        QueryResult queryMessage = Reflect.on(mqAdminImpl).call("queryMessage", topic, msgId, 32,
                MessageClientIDSetter.getNearlyTimeFromID(msgId).getTime() - 1000 * 60 * 60 * 13L, Long.MAX_VALUE, true).get();
        if (queryMessage != null
                && queryMessage.getMessageList() != null
                && !queryMessage.getMessageList().isEmpty()) {
            return queryMessage.getMessageList().get(0);
        }
        return null;
    }
